...
class LLMIntegration:
    ...
    def __init__(self, api_type: str = 'gpt', base_url: Optional[str] = None, api_key: Optional[str] = None, dataset=None, metadata=None):
        self.api_type = api_type
        if api_type == 'ollama':
            self.client = OpenAI(base_url=base_url or 'http://localhost:11434/v1', api_key='ollama')
    ...
    def execute_function(self, function_name: str, function_args: Dict[str, Any]) -> Any:
        ...
        try:
            ...
            elif function_name.startswith("plot_") or function_name.startswith("perform_"):
                plot_function = getattr(self.dataset, function_name.split('.')[-1], None)
                if plot_function:
                    return plot_function(**function_args)
            raise ValueError(f"Function {function_name} not implemented or dataset not available")
        except Exception as e:
            return f"Error executing {function_name}: {str(e)}"

    def handle_function_calls(self, tool_calls: List[Any], ) -> Dict[str, Any]:
        """
        ...
        funcs_and_args = '\n'.join([f"Calling function: {tool_call.function.name} with arguments: {tool_call.function.arguments}" for tool_call in tool_calls])
        self.messages.append({
                "role": "assistant", 
                "content": funcs_and_args,
        ...
        for tool_call in tool_calls:
            ...
            self.messages.append({
                "role": "tool",
                "content": json.dumps({"result": str(function_result), "artifact_id": artifact_id}),
    ...
    def chat_completion(self, prompt: str, role: str = "user") -> Tuple[str, Dict[str, Any]]:
        """
        ...
        try:
            ...
            if parsed_response['tool_calls']:
                parsed_response = self.handle_function_calls(parsed_response['tool_calls'])
                new_artifacts = parsed_response.pop('new_artifacts', {})
            
            self.messages.append({"role": "assistant", "content": parsed_response['content']})
    ...
    def switch_backend(self, new_api_type: str, base_url: Optional[str] = None, api_key: Optional[str] = None):
        """
        ...
        None
        """
        self.__init__(api_type=new_api_type, base_url=base_url, api_key=api_key, dataset=self.dataset, metadata=self.metadata)
        ...
        for i, message in enumerate(self.messages):
            ...
            if role == 'Assistant' and 'tool_calls' in message:
                ...
                if artifact_id and artifact_id in self.artifacts:
                    ...
                    display(Markdown(f"**Function Result** (Artifact ID: {artifact_id}):"))
