...
def draw_predefined_workflow(workflow = ["remove contaminations", "subset data", "log2 transform"]):
    available_steps = ["remove contaminations", "remove samples", "subset data", "filter data completeness", "log2 transform", "normalization", "imputation", "batch correction"]
...
def configure_preprocessing():
    ...
    remove_samples = st.multiselect(
        ...
        options=[] if 'dataset' not in st.session_state else st.session_state.dataset.metadata[
    ...
    batch = st.selectbox(
            "Batch", options=[False] if 'dataset' not in st.session_state else [False] + st.session_state.dataset.metadata.columns.to_list()
        )

    return remove_contaminations, remove_samples, subset, data_completeness, log2_transform, normalization, imputation, batch

def update_workflow(remove_contaminations, remove_samples, subset, data_completeness, log2_transform, normalization, imputation, batch):
    old_workflow = st.session_state.workflow
    st.session_state.workflow = [el for el, form in zip(["remove contaminations", "remove samples", "subset data", "filter data completeness", "log2 transform", "normalization", "imputation", "batch correction"], [remove_contaminations, remove_samples, subset, data_completeness, log2_transform, normalization, imputation, batch]) if form not in [None, False, [], 0.0]]
    if old_workflow != st.session_state.workflow:
        st.rerun()

def run_preprocessing(remove_contaminations, remove_samples, subset, data_completeness, log2_transform, normalization, imputation, batch):
